...
set resultado= concat("La tabla películas contiene ", numPeliculas, " películas, con una duración media de ", 
mediaDurac, " minutos, y notas entre ", notaMin, " y " , notaMax);
...
set resultado = concat("La pelicula con menor nota de las 2 es ",(select Nombre from Peliculas where ID = ID1),
" con un ",(select Nota from Peliculas where ID = ID1), " sobre 10");
elseif((select Nota from Peliculas where ID = ID1)>(select Nota from Peliculas where ID= ID2))then
set resultado = concat("La pelicula con menor nota de las 2 es ",(select Nombre from Peliculas where ID = ID2), 
" con un ",(select Nota from Peliculas where ID = ID2), " sobre 10");
else
...
/*Ejercicio 3.- Crea una función en MySQL que a partir del Genero de la película insertado por el 
usuario devuelva el número de películas de ese género contenidas en la tabla, siempre y cuando existan 
más de 2 películas de dicho genero en la base de datos.
...
el nombre de las películas (concatenadas) contenidas en ese límite. El primer dato que inserte el usuario corresponderá
al número de saltos (número de registros a omitir) que desea dar el usuario. El segundo dato corresponderá al número de
registros que se desean mostrar.
...
if (numSaltos + numRegistros)>=numPeliculas then
set resultado="No hay tantos registros en la base de datos";
else

set contador=contador+numSaltos;
while contador <= nunPeliculas do
...
select ejercicio4(1,4);

/*PROCEDIMIENTOS

Ejercicio 5.- Crea un procedimiento en MySQL que a partir de un dato dado por el usuario añada un nuevo registro
a la tabla Películas con los siguientes datos específicos.
...
/*Ejercicio 6.- Crea un procedimiento en MySQL que añada una nueva columna a la tabla Películas con las características que el
usuario elija. El usuario insertará 4 datos en la ejecución del procedimiento, nombre de la columna, tipo de dato, número de 
caracteres y valor por defecto.
...
Ejercicio 7.- Crea un procedimiento en MySQL que a partir de un ID insertado por el usuario, modifique el nombre del director
de dicha película. El nuevo nombre del director será el mismo nombre que tenía de derecha a izquierda. Muestra la tabla en el
...
Ejercicio 8.- Crea un procedimiento en MySQL que a partir de dos datos insertados por el usuario realice una consulta SQL
a la tabla Películas. El usuario insertará 4 datos en la ejecución del procedimiento, nombre columna y valor columna.
...
El procedimiento mostrará todos los datos de la tabla Películas ordenador alfabéticamente por el 
Nombre de las películas, sin incluir la película “Bright”*/